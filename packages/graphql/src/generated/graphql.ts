...
export type Zc_AdventureLevelCardFragment = { id: number, xxHash: string, adventure: boolean, dateCreated: unknown, levelPoints: { points: number, rating: number } | null, records: { totalCount: number }, personalBestGlobals: { totalCount: number }, votes: { totalCount: number }, worldRecordGlobal: { record: { time: number } | null, user: { steamId: unknown, steamName: string | null } | null } | null, levelItems: { nodes: Array<{ name: string, imageUrl: string, fileUid: string, fileAuthor: string, workshopId: unknown, validationTimeAuthor: number, validationTimeGold: number, validationTimeSilver: number, validationTimeBronze: number, author: { steamId: unknown, steamName: string | null } | null }> } };
