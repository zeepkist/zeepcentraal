...
export function startOfDayInTimeZone(date: Date, timeZone: string): Date {
	...
	return wallClockStartInTimeZone(Date.UTC(year, month - 1, day), timeZone)
}
...
export function startOfYearInTimeZone(date: Date, timeZone: string): Date {
	const { year } = dateParts(date, timeZone)
	return wallClockStartInTimeZone(Date.UTC(year, 0, 1), timeZone)
}
