<template>
	<div class="space-y-4">
		<div v-for="series in seriesModels" :key="series.group" class="rounded-2xl border border-border bg-gradient-to-br from-card to-primary/5 p-3 sm:p-4">
			...
			<LineChart :data="series.data" :categories="{ value: { name: series.title, color: series.color } }" :y-axis="['value']" :height="220" :x-formatter="(index: number) => formatDate(series.data[index]?.date)" :y-formatter="series.inverted ? formatRankTick : undefined" :line-width="3" :hide-legend="true" :duration="duration" :tooltip="tooltipOptions">
...
const pointsOptions = computed(() => [
	...
	{ value: 'totalPoints' as const, label: props.labels.totalPoints, icon: 'sum' },
])
const standingOptions = computed(() => [
	{ value: 'rank' as const, label: props.labels.rank, icon: 'trophy' },
	{ value: 'worldRecords' as const, label: props.labels.worldRecords, icon: 'flag' },
])
const seriesModels = computed(() => [
	pointsSeries.value === 'rankedPoints'
		...
		: { group: 'points' as const, key: 'totalPoints', title: props.labels.totalPoints, description: props.labels.totalPointsDescription, color: '#a855f7', inverted: false, data: props.history.map((point) => ({ date: point.date, value: point.totalPoints })) },
	standingSeries.value === 'rank'
		...
		: { group: 'standing' as const, key: 'worldRecords', title: props.labels.worldRecords, description: props.labels.worldRecordsDescription, color: '#f43f5e', inverted: false, data: props.history.map((point) => ({ date: point.date, value: point.worldRecords })) },
])
