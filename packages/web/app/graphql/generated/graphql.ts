...
export type Zc_AdventureLevelsQuery = { levels: { nodes: Array<{ id: number, xxHash: string, adventure: boolean, dateCreated: unknown, levelPoints: { points: number, rating: number, modifierPopularity: number } | null, records: { totalCount: number }, personalBestGlobals: { totalCount: number }, levelItems: { nodes: Array<{ name: string, imageUrl: string, author: { steamId: unknown, steamName: string | null } | null }> } }> } | null };
