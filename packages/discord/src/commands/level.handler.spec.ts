...
test('level rejects missing and private results', async () => {
	...
	expect(handleLevel(first, missing)).rejects.toThrow('Public level not found.')
	...
	expect(handleLevel(second, privateContext)).rejects.toThrow('Level is not publicly visible.')
})
...
test('level autocomplete handles short and object focus values with a 25-item cap', async () => {
	...
	await autocompleteLevel(short.interaction, context)
	...
	await autocompleteLevel(result.interaction, searchable)
