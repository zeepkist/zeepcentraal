...
test('user resolves linked Discord target and renders ranked profile', async () => {
	...
	await handleUser(interaction, context)
...
test('user reports missing identifier and unlinked Discord accounts', async () => {
	...
	expect(handleUser(byId, missing)).rejects.toThrow('Player not found.')
	...
	expect(handleUser(byDiscord, unlinked)).rejects.toThrow('Link account first')
})
...
test('user reports profile removed after lookup', async () => {
	...
	expect(handleUser(interaction, context)).rejects.toThrow('Player not found.')
})
