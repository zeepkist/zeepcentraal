...
test('locks sorted unique levels and uses one transaction executor throughout', async () => {
	...
	expect(lockQuery.sql).toContain('pg_advisory_xact_lock($1, target.id)')
	expect(lockQuery.sql).toContain('ORDER BY target.id')
	expect(lockQuery.params).toEqual([1_861_284_954, '{2,7}'])
