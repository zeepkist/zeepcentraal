...
export async function getOrInsertUsersBulk(steamIds: string[]): Promise<Map<string, number>> {
	...
	const existingUsers = await db
		...
		.from(user)
		.where(inArray(user.steamId, uniqueSteamIds.map((value) => BigInt(value))));
	...
	if (missingSteamIds.length > 0) {
		const steamProfiles = await Promise.all(missingSteamIds.map((steamId) => getSteamUser(steamId)));
		...
		if (unresolvedSteamIds.length > 0) {
			const resolvedUsers = await db
				.select({ id: user.id, steamId: user.steamId })
				.from(user)
				.where(inArray(user.steamId, unresolvedSteamIds.map((value) => BigInt(value))));
