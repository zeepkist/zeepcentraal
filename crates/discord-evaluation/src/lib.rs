...
impl Sessions {
    ...
    pub fn put(&mut self, id: u64, owner: String, now: u64, value: Value) {
        ...
        self.pages.push_back((
            id,
            owner,
            now + zc_core::environment::var("ZC_DISCORD_EVAL_TTL_MS")
                .ok()
...
/// Subscription protocol reducer used by replay tests. Cursor is persisted by the backend,
/// never inferred from the newest subscription item before successful delivery.
#[derive(Default)]
...
impl Subscription {
    ...
    pub fn receive(&mut self, packet: &Value, query: &str) -> anyhow::Result<Option<Value>> {
        match packet["type"].as_str() {
            Some("connection_ack") => {
                self.ready = true;
                Ok(Some(
                    json!({"id":"activity", "type":"subscribe", "payload":{"query":query, "operationName":"ZC_DiscordActivityEventsLive"}}),
                ))
            }
            Some("ping") => Ok(Some(json!({"type":"pong", "payload":packet["payload"]}))),
            Some("next") => {
                anyhow::ensure!(
                    self.ready && packet["payload"].get("errors").is_none(),
    ...
    pub fn cancel(&mut self) -> Value {
        self.ready = false;
        json!({"id":"activity", "type":"complete"})
    }
}
...
mod tests {
    ...
    #[test]
    fn subscription_reconnect_and_cancel() {
        let mut stream = Subscription::default();
        assert_eq!(stream.opened()["type"], "connection_init");
        assert_eq!(
            stream
                .receive(&json!({"type":"connection_ack"}), "subscription test")
                .unwrap()
                .unwrap()["type"],
            "subscribe"
        );
