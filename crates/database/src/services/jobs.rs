...
impl Database {
    ...
    pub async fn recalculate_all_player_scores(&self) -> Result<()> {
        ...
        connection.transaction::<(), anyhow::Error, _>(|connection| Box::pin(async move {
            sql_query(
                "WITH activity AS MATERIALIZED (SELECT id_user,max(date_created) AS latest FROM public.record GROUP BY id_user), \
                 inactive AS MATERIALIZED (SELECT id_user FROM activity WHERE latest<clock_timestamp()-interval '6 months'), \
                 reset_contributions AS (UPDATE public.user_point_contribution contribution SET player_decayed_points=0,date_calculated=clock_timestamp() FROM inactive WHERE contribution.id_user=inactive.id_user), \
                 reset_points AS (UPDATE public.user_points points SET points=0,rank=-1,date_updated=clock_timestamp() FROM inactive WHERE points.id_user=inactive.id_user), \
                 ...
                 updated AS (UPDATE public.user_point_contribution target SET contribution_rank=ranked.contribution_rank,player_decayed_points=target.level_decayed_points::double precision*POWER(0.95,ranked.contribution_rank-1),date_calculated=clock_timestamp() FROM ranked WHERE target.id_user=ranked.id_user AND target.id_level=ranked.id_level RETURNING target.id_user,target.player_decayed_points,target.level_decayed_points), \
                 totals AS (SELECT id_user,ROUND(SUM(player_decayed_points))::integer AS points,ROUND(SUM(level_decayed_points))::integer AS total_points FROM updated GROUP BY id_user) \
                 INSERT INTO public.user_points(id_user,points,total_points,world_records,date_created,date_updated) \
