...
async fn adapter_and_migration_contract() -> anyhow::Result<()> {
    ...
    let db = Database::connect(&config.database_url, 3).await?;
    ...
    let before: i64 = client
        .query_one("SELECT count(*) FROM public.record", &[])
        .await?
        .get::<_, i64>(0);
    ...
    assert_eq!(client.query_one("SELECT count(*) FROM public.record r JOIN public.record_audit a ON a.id_record=r.id WHERE r.time=27.125", &[]).await?.get::<_, i64>(0),1);
    // Fail the SECOND statement to establish transaction rollback, not just FK validation.
    client.batch_execute("CREATE FUNCTION public.reject_preview_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'evaluation rollback'; END $$; CREATE TRIGGER reject_preview_audit BEFORE INSERT ON public.record_audit FOR EACH ROW EXECUTE FUNCTION public.reject_preview_audit();").await?;
    let failed = db.submit(1, 1, 26.0).await;
    client.batch_execute("DROP TRIGGER reject_preview_audit ON public.record_audit; DROP FUNCTION public.reject_preview_audit();").await?;
    assert!(failed.is_err());
    assert_eq!(
        client
            .query_one("SELECT count(*) FROM public.record WHERE time=26.0", &[])
            .await?
            .get::<_, i64>(0),
        0
    );
    client.batch_execute("DELETE FROM public.record_audit WHERE id_record IN (SELECT id FROM public.record WHERE time=27.125); DELETE FROM public.record WHERE time=27.125;").await?;
    drop(client);
    connection.await?;
    Ok(())
}
